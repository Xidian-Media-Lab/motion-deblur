...
class laplacianGrad(nn.Module):
    def __init__(self):
        super(laplacianGrad, self).__init__()

        self.laplacian = kornia.filters.laplacian
        self.l1_loss = nn.L1Loss()

    def forward(self, fused, vis, nir):
        nir_grad = torch.abs(self.laplacian(nir, 3))
        vis_grad = torch.abs(self.laplacian(vis, 3))
        fused_grad = torch.abs(self.laplacian(fused, 3))

        loss = self.l1_loss(torch.max(nir_grad, vis_grad), fused_grad)
